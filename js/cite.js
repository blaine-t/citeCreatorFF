...
function computeCite(strNewName, strNewQuals, strNewDate, strNewTitle, strNewPublication) {
	...
	divClose.id = 'cite_close';
	divClose.innerHTML = 'x';
	div.appendChild(divClose);
	//Include rating unless Suppress Rating is set
	if (objSettings.suppressrating != 1) {
		divRating = document.createElement('div');
		divRating.id = 'cite_rating';
		divRating.classList.add(strRatingClass);
		divRatingSpan = document.createElement('span')
		divRatingSpan.innerText = strRating;
		divRating.appendChild(divRatingSpan);
		div.appendChild(divRating);
	}
	br = document.createElement('br')
	div.appendChild(br)
	divContent = document.createElement('div');
	divContent.innerText = strCite;
	divContent.id = 'cite_content';
